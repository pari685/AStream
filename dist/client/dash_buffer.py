...
PLAYER_STATES = ['INITIAL_BUFFERING', 'PLAY', 'PAUSE', 'BUFFERING', 'STOP', 'END']
...
class DashBuffer:
    ...
    def set_state(self, state):
        ...
        if state in PLAYER_STATES:
            ...
            dash_config.LOG.info("State set to {}".format(state))
        else:
            dash_config.LOG.error("Unidentified state: {}".format(state))
        ...
        while True:
            ...
            if self.playback_state == "INITIAL_BUFFERING":
                if self.buffer.qsize() * SEGMENT_DURATION < INITIAL_BUFFERING_DURATION:
                    ...
                    dash_config.LOG.info("Initial Waiting Time = {}".format(initial_wait))
            ...
            if self.playback_state == "PLAY":
                    ...
                    while time.time() < future:
                        ...
                        if not self.playback_start_time:
                            ...
                            dash_config.LOG.info("Started playing with representation {}".format(
                                segment['bitrate']))
                        ...
                        if self.current_playback_time < int_time:
                            ...
                            dash_config.LOG.error("Playback time ahead")
                        if self.current_playback_time >= self.playback_duration:
                            dash_config.LOG.info("Completed the video playback: {} seconds".format(self.current))
                            return 1
                    else:
    ...
    def start(self):
        """ Start playback"""
        self.playback_state = "INITIAL_BUFFERING"
        dash_config.LOG.info("Starting the playback")
        self.player_thread = threading.Thread(target=initialize_player)
